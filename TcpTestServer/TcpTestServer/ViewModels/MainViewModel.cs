...
using TcpTestServer.Helpers;

namespace TcpTestServer.ViewModels;
...
#pragma warning restore MVVMTK0045 // Using [ObservableProperty] on fields is not AOT compatible for WinRT
    ...
    private void TcpService_MessageSend(object? sender, string e)
    {
        ...
        try
        {
            ...
            ShowInfoMessage($"Received: {ReceivedMessage}\nSend: {SendMessage}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "", []);
    ...
    private void Disconnect()
    {
        ...
        try
        {
            DisconnectAndInit(DisconnectionCause.ServerStopped);
        }
        catch (Exception ex)
        {
            ShowError(ex);
